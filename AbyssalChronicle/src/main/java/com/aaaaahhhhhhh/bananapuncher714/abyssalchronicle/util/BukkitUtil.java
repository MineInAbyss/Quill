package com.aaaaahhhhhhh.bananapuncher714.abyssalchronicle.util;
...
public class BukkitUtil {
	...
	public static PluginCommand constructCommand( Plugin plugin, String id ) {
		PluginCommand command = null;
		try {
			command = PLUGINCOMMAND_CONSTRUCTOR.newInstance( id, plugin );
	 ...
	 */
	public static void callEventSync( Plugin plugin, Event event ) {
		if ( Bukkit.getServer().isPrimaryThread() ) {
			Bukkit.getPluginManager().callEvent( event );
		} else {
			Bukkit.getScheduler().scheduleSyncDelayedTask( plugin, () -> { callEventSync( plugin, event ); } );
		}
	}
